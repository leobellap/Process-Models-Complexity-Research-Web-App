...
function preloadImages() {
  for (var step = 1; step <= gridSize; step++) {
    for (var difficulty = 1; difficulty <= gridSize; difficulty++) {
      var src = `/static/images/models/Dataset_5/Dataset-5_AR-${step}0_PR-(0-100)/Dataset-5_AR-${step}0_PR-${difficulty}0.dot.png`;
...
function updateImage(x, y) {
  var src = `/static/images/models/Dataset_5/Dataset-5_AR-${x}0_PR-(0-100)/Dataset-5_AR-${x}0_PR-${y}0.dot.png`;
  if (imageCacheIntOp[src]) {
